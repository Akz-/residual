...
void GfxOpenGL::drawTextObject(TextObject *text) {
	...
	for (int j = 0; j < numLines; ++j) {
		...
		for (uint i = 0; i < line.size(); ++i) {
			...
			glBegin(GL_QUADS);
			glTexCoord2f(cx, cy);
			glVertex2i(toScreenX(z), toScreenY(w));
			glTexCoord2f(cx + width, cy);
			glVertex2i(toScreenX(z + size), toScreenY(w));
			glTexCoord2f(cx + width, cy + width);
			glVertex2i(toScreenX(z + size), toScreenY(w + size));
			glTexCoord2f(cx, cy + width);
			glVertex2i(toScreenX(z), toScreenY(w + size));
			glEnd();
